...
- Acquire multi-month data to test seasonality and validate the forecasting model out-of-time.
- Apply class-balancing techniques (SMOTE, class weighting review) to improve minority payment-type prediction.
- Formalize cluster count selection using elbow/silhouette methods before using segments operationally.
- Build a lightweight Power BI/Tableau dashboard on top of the cleaned PostgreSQL tables for non-technical stakeholders.